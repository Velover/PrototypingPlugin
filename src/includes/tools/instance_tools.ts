...
declare global {
	type InstancePropertiesList<T extends Instance> = {
...
export namespace InstanceTools {
	...
	export function AsignProperties<T extends Instance>(
		instance: T,
		properties: InstancePropertiesList<T>,
	) {
	 ...
	 */
	export function LoadSettingsFromModuleScript<T extends {}>(
		...
		for (const [name, default_setting] of default_settings as unknown as Map<
			keyof T,
			unknown
		>) {
	...
	export function GetPlayerCharacterFromInstace(
		...
		const character = FindAncestor(part, (instance) => {
			return (
				instance.IsA("Model") &&
				Players.GetPlayerFromCharacter(instance) !== undefined
			);
		});
	...
	export function SearchToolInPlayer(player: Player, tool_name: string) {
		const search_function = (instance: Instance) =>
			instance.IsA("Tool") && instance.Name === tool_name;
	...
	export function WaitForPath<T extends Instance = Instance>(
		...
		for (const name of path) {
			...
			assert(
				child !== undefined,
				`${previous.GetFullName()} doesnt have parent`,
			);
		}
		return child as T;
	}

	export function FindFirstOnPath<T extends Instance = Instance>(
		instance: Instance,
		path: string[],
	) {
	...
	export function DefineItem<T extends keyof CreatableInstances>(
		name: string,
		parent: Instance,
		class_name: T,
		properties: InstancePropertiesList<CreatableInstances[T]>,
		...
		const instance = FunctionTools.ExecuteOnServerAndClient(
			() => {
				assert(
					parent.FindFirstChild(name) === undefined,
					`Instance with name ${name} is already in ${parent.GetFullName()}`,
				);
	...
	export function GetOrCreate<T extends keyof CreatableInstances>(
		...
		let instance = parent.FindFirstChild(name) as

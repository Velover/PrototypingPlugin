export namespace DebuggingTools {
	...
	export namespace JSONAnalizer {
		...
		function AddToRecursiveTable(
			origin_table: RecursiveTable,
			new_table: RecursiveTable,
		) {
			...
			if (is_array) {
				...
				for (const value of data as DefinedArray) {
					...
					if (typeIs(value, "table")) {
						const recursive_table = AnalizeJSON(
							value as DefinedTable,
		...
		function ConvertRecursiveTableToRecursiveMap(
			recursive_table: RecursiveTable,
		) {
			...
			for (const [key, [types, optional]] of recursive_table) {
				...
				for (const type_name of types) {
					if (typeIs(type_name, "table")) {
						...
						types_list.push(
							ConvertRecursiveTableToRecursiveMap(type_name as RecursiveTable),
						);
						continue;
					}
		...
		export function AnalizeAndReturnJson(
			...args: Parameters<typeof AnalizeJSON>
		) {
			const recursive_table = AnalizeJSON(...args);
			const recursive_map =
				ConvertRecursiveTableToRecursiveMap(recursive_table);
			return recursive_map;
		}

		export function AnalizeAndPrintJson(
			...args: Parameters<typeof AnalizeAndReturnJson>
		) {

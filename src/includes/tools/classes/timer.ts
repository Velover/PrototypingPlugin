...
export class Timer {
	static Create(
		callback: (
			timer: Timer,
			GetConnection: () => RBXScriptConnection | undefined,
		) => {
			wait_time?: number;
			one_shot?: boolean;
			autostart?: boolean;
		...
		const { wait_time, one_shot, autostart, timeout_callback } = callback(
			timer,
			() => connection,
		);
		...
		if (timeout_callback)
			connection = timer.on_time_out_.Connect(timeout_callback);
...
export class CallbackTimer {
	static Create(
		callback: (timer: CallbackTimer) => Partial<{
			wait_time: number;
			one_shot: boolean;
			autostart: boolean;
			timeout_callback: () => void;
		...
		if (yield_after_call !== undefined)
			timer.yield_after_call_ = yield_after_call;
	...
	GetTimeLeft() {
		return math.max(this.wait_time_ - (os.clock() - this.start_time_), 0);
	}
...
export class FrameTimer {
	...
	static Create(
		callback: (
			...
			one_shot?: boolean;
			autostart?: boolean;
			update_callback?: FrameTimerCallback;
			timeout_callback?: () => void;
		},
	) {
	...
	static CreateFast(
		callback: (timer: FrameTimer) => {
			...
			update_callback: FrameTimerCallback;
			yields?: boolean;
			destroy_after_ended?: boolean;
		},
	) {
		const timer = new FrameTimer();
		const { wait_time, update_callback, yields, destroy_after_ended } =
			callback(timer);
		timer.SetCallback(update_callback);
		if (destroy_after_ended ?? true)
			timer.on_time_out_.Once(() => timer.Destroy());
	...
	static CreateTween<T extends Tweenable>(
		...
		timer.SetCallback((time_passed) => {
			...
			const new_value = TweenTools.TweenValue(
				start_value,
				target_value,
				alpha,
				easing_style,
	...
	private stepping_event_: RBXScriptSignal<(delta_time: number) => void> =
	...
	SetSteppingEvent(
		stepping_event: RBXScriptSignal<(delta_time: number) => void>,
	) {

...
export namespace TweenTools {
	...
	export function LerpValue<T extends CustomTweenable>(
		start_value: T,
		target_value: T,
		alpha: number,
	): T {
		...
		if (value_type === "number") {
			...
			return new UDim(
				Lerp(value_0.Scale, value_1.Scale, alpha),
				Lerp(value_0.Offset, value_1.Offset, alpha),
			) as T;
		} else if (value_type === "Rect") {
			const value_0 = <Rect>start_value;
			const value_1 = <Rect>target_value;
			return new Rect(
				value_0.Min.Lerp(value_1.Min, alpha),
				value_0.Max.Lerp(value_1.Max, alpha),
			) as T;
		} else if (value_type === "boolean") {
			return (alpha >= 0.5 ? true : false) as T;
		}
		...
		return value_0.add(
			new Vector2int16(direction.X * alpha, direction.Y * alpha),
		) as T;
	}

	export function TweenValue<T extends CustomTweenable>(
		start_value: T,
		target_value: T,
		alpha: number,
		easing_style: CastsToEnum<Enum.EasingStyle> = Enum.EasingStyle.Sine,
		easing_direction: CastsToEnum<Enum.EasingDirection> = Enum.EasingDirection
			.In,
	): T {
		const new_alpha = TweenService.GetValue(
			alpha,
			easing_style,
			easing_direction,
		);

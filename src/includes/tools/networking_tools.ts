...
export namespace NetworkingTools {
	...
	export function DefineRemoteEvent<T extends Callback = Callback>(
		name: string,
		parent: Instance = network_folder,
	) {
		const remote = FunctionTools.ExecuteOnServerAndClient(
			() => {
				...
				assert(
					...
					`Failed to create RemoteEvent. Instance with name: ${name} already exist in ${parent.GetFullName()}`,
				);
	...
	export function DefineRemoteFunction<T extends Callback = Callback>(
		...
		const remote = FunctionTools.ExecuteOnServerAndClient(
			() => {
				...
				assert(
					parent.FindFirstChild(name) === undefined,
					`Failed to create RemoteFunction. Instance with name: ${name} already exist in ${parent.GetFullName()}`,
				);

...
export namespace DetectingTools {
	...
	function GetCorners(part: BasePart, local_space?: boolean) {
		...
		for (const corner of corners) {
			...
			part_corners.push(
				local_space ? part_corner : part.CFrame.mul(part_corner),
			);
		}
	...
	export function GetPointUnderThePlayer(
		player: Player,
		max_depth: number,
		filter_not_anchored: boolean = true,
	) {
		if (player.Character === undefined) return;
		const player_position = player.Character.GetPivot().Position;

		//ignores the players characters
		const instances_to_ignore = new Array<Instance>();
		for (const player of Players.GetPlayers()) {
			if (player.Character === undefined) continue;
			instances_to_ignore.push(player.Character);
		}

		//ignores not anchored parts
		if (filter_not_anchored) {
			const parts_around = Workspace.GetPartBoundsInRadius(
				player_position,
				max_depth,
			);
			for (const part of parts_around) {
				if (part.Anchored) continue;
				instances_to_ignore.push(part);
			}
		}

		const raycast_parameters = new RaycastParams();
		raycast_parameters.FilterDescendantsInstances = instances_to_ignore;
		const raycast_result = Workspace.Raycast(
			player_position,
			Vector3.yAxis.mul(-1 * max_depth),
			raycast_parameters,
		);

		return raycast_result?.Position ?? player_position;
	}
	...
	function GetFilteredPointsFromScreenPoints(
		screen_points: [Vector3, boolean][],
	) {
	...
	export function IsOnTheScreen(
		...
		const screen_points = PointsToViewPortPosition(camera, [
			...part_corners,
			...sampled_points,
		]);

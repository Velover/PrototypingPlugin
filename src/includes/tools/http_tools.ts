...
export namespace HttpTools {
	...
	export function FetchJSONFromLink<ReturnType>(
		...
		try {
			...
			result = HttpService.JSONDecode(
				remove_comments ? RemoveJSONComments(raw_data) : raw_data,
			);
		} catch (error) {
			success = false;
	 ...
	 */
	export function ConstructGetRequest<Data extends {} = {}>(
		domain: string,
		path: string,
		default_parameters?: MakeOnlyOptionalKeys<Data>,
	) {
		...
		return FunctionTools.CreateFunctionWithOptionalArguments<
			Data,
			Promise<string>
		>(default_parameters, SetParameters);
	}
}

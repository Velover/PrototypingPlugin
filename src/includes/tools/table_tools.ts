...
type MutableInterface<T extends {}> = { - readonly [key in keyof T]: T[key] extends AnyData ? Mutable<T[key]> : T[key] }
type MutableMap<T extends AnyMap> = T extends AnyMap<infer K, infer V> ? Map<K, V extends AnyData ? Mutable<V> : V> : never
type MutableArray<T extends AnyArray> = T extends AnyArray<infer V> ? Array<V extends AnyData ? Mutable<V> : V> : never

type ImmutableInterface<T extends {}> = { readonly [key in keyof T]: T[key] extends AnyData ? Immutable<T[key]> : T[key] }
type ImmutableMap<T extends AnyMap> = T extends AnyMap<infer K, infer V> ? Map<K, V extends AnyData ? Immutable<V> : V> : never
type ImmutableArray<T extends AnyArray> = T extends AnyArray<infer V> ? Array<V extends AnyData ? Immutable<V> : V> : never
...
declare global {
  ...
  type Mutable<T extends AnyData> =
    T extends AnyMap ? MutableMap<T> :
    T extends AnyArray ? MutableArray<T> :
    T extends {} ? MutableInterface<T> :
    never;

  type Immutable<T extends AnyData> =
    T extends AnyMap ? ImmutableMap<T> :
    T extends AnyArray ? ImmutableArray<T> :
...
export namespace TableTools {
   ...
   */
  export function CopyData<Data extends AnyData>(data: Data, callback: (draft: Mutable<Data>) => void): Data {
    //clones the data
  ...
  export function DeepCopyData<Data extends AnyData>(data: Data, callback?: (draft: Mutable<Data>) => void): Data {
  ...
  export function FillOutDefaults<T extends {}>(data: Partial<T>, default_data: T) {
    return { ...default_data, ...data }
  }

  type GetKeyType<T extends AnyMap> =
    T extends Map<infer K, defined> ? K :
    T extends ReadonlyMap<infer K, defined> ? K :
    never;

  type GetValueType<T extends AnyMap> =
    T extends Map<defined, infer V> ? V :
    T extends ReadonlyMap<defined, infer V> ? V :
    never;
  ...
  export function ExtractElementsFromRecursiveData<Data extends AnyData>(data: Data, check: (element: defined, key: defined, taken_elements: Array<defined>, data: Data) => boolean, ignore_table_if_added: boolean = true) {
  ...
  export function Filter<K, V>(map: Map<K, V>, check: (value: V, key: K, map: Map<K, V>) => boolean) {

...
export default function DropDownMenu() {
  ...
  const [anchor_point, SetAnchorPoint] = useState(new Vector2(.5, 0));
  ...
  useEffect(() => {
    ...
    const top = absolute_position.add(absolute_size.mul(new Vector2(.5, 0)));
    const bottom = absolute_position.add(absolute_size.mul(new Vector2(.5, 1)));
    ...
    const expected_size_y = math.min(scrolling_frame.AbsoluteCanvasSize.Y, max_y_size);
    ...
    const fill_direction = offset_from_bottom < 0 ? EFillDirection.Top : EFillDirection.Bottom
    ...
    SetPosition(fill_direction === EFillDirection.Top ? bottom_position : top_position);
    SetAnchorPoint(fill_direction === EFillDirection.Top ? new Vector2(.5, 0) : new Vector2(.5, 1));

    const canvas_size_y_vector = new Vector2(0, scrolling_frame.AbsoluteCanvasSize.Y);
    SetCanvasPosition(fill_direction === EFillDirection.Bottom ? canvas_size_y_vector : Vector2.zero)
  }, [track_button]);

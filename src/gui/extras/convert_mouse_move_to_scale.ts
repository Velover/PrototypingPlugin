...
export type MouseMovedCallback<T extends GuiBase2d = GuiBase2d> = (scale: Vector2, gui_element: T) => void

export default function ConvertMouseMoveToScale<T extends GuiBase2d = GuiBase2d>(callback: MouseMovedCallback<T>) {
  return (gui_element: T, x: number, y: number) => {
    ...
    callback(Vector2Tools.Clamp(relative_position.div(absolute_size), 0, 1), gui_element);
  }
}
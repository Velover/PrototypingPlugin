...
GuiState.selected_mesh(
	<Meshes.EMesh>plugin.GetSetting(mesh_save_name) ?? Meshes.EMesh.box,
);
GuiState.selected_color(
	<Textures.EColor>plugin.GetSetting(color_save_name) ?? Textures.EColor.Orange,
);
GuiState.selected_texture(
	<Textures.ETexture>plugin.GetSetting(texture_save_name) ??
		Textures.ETexture.Texture02,
);
